...
export default function AttributeTable({ layer, isOpen, onClose }: AttributeTableProps) {
  ...
  const fetchLayerData = useCallback(async (query: string) => {
    setIsLoading(true)
    try {
          ...
          <Card className={`rounded-md border border-border py-2 ${isLoading ? "animate-pulse" : ""}`}>
            <CardContent className="px-2">
              <pre className="font-medium text-orange-500 overflow-x-auto">
                <code className="text-sm">{sqlQuery} {duration > 0 && <span className="text-muted-foreground">{(sqlQuery.indexOf('\n') == -1) && <br />}&nbsp;→ {data.data.length} rows in {Math.round(duration)}ms</span>}</code>
              </pre>
            </CardContent>
          </Card>
              ...
              <thead>
                <tr className="border-b border-border">
                  {data.columns.map((column, i) => (
                    <th key={i} className="p-2 text-left font-medium text-muted-foreground bg-background sticky top-0 border-b border-border">
                      {column}
                    </th>
                  ))}
                </tr>
